...
### The Stack
The local Supabase stack (`bun run db:start`) spins up the following services via Docker:
...
```
*Note: You will be asked for your database password. If you don't have it, ask the team lead.*
...
bun run db:start
```
*This may take a few minutes the first time as it pulls Docker images.*
...
3.  Test locally (`bun run db:reset`).
...
3.  Run `bun run db:reset` to apply both.
...
- [ ] Migration runs successfully locally (`bun run db:reset`)
...
5.  **Test Locally:** Run `bun run db:reset` to ensure the squashed migration works.
...
2.  `bun run db:reset` (Resets DB, runs migrations from this branch, seeds data)
3.  `bun run dev`

*Note: `db:reset` destroys local data. Ensure your `seed.sql` provides enough test data.*
...
| :--- | :--- | :--- |
| `db:start` | `supabase start` | Start local Supabase stack |
| `db:stop` | `supabase stop` | Stop local services |
| `db:status` | `supabase status` | Show service URLs and keys |
| `db:reset` | `supabase db reset` | Wipe DB, apply migrations, run seed |
| `db:seed` | `supabase db reset` | Alias for reset |
| `db:gen:types` | `supabase gen types... --local` | Generate TS types from LOCAL DB |
| `gen:types` | `supabase gen types...` | Generate TS types from PROD DB |
...
**Fix:** Check your migration order. Ensure you aren't referencing a table created in a *later* migration.

### Next.js Can't Connect
**Error:** `ECONNREFUSED 127.0.0.1:54321`
**Fix:**
1.  Run `bun run db:status` to confirm Supabase is running.
...
bun run db:reset  # Apply to local
```
...
*Detailed instructions will be added here upon transition.*
