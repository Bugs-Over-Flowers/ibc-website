...
BEGIN
  ...
  ) AND to_regclass('storage.objects') IS NOT NULL AND NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'objects_delete_delete_prefix'
      AND tgrelid = to_regclass('storage.objects')
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'objects_update_create_prefix'
      AND tgrelid = to_regclass('storage.objects')
  ) THEN
...
BEGIN
  ...
  ) AND to_regclass('storage.prefixes') IS NOT NULL AND NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'prefixes_create_hierarchy'
      AND tgrelid = to_regclass('storage.prefixes')
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'prefixes_delete_hierarchy'
      AND tgrelid = to_regclass('storage.prefixes')
  ) THEN
