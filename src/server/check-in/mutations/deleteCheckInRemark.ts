...
import {
  DeleteCheckInRemarkInput,
  type DeleteCheckInRemarkOutput,
} from "@/lib/validation/check-in/server";

export async function deleteCheckInRemark(
  input: DeleteCheckInRemarkInput,
): Promise<DeleteCheckInRemarkOutput> {
  const parsed = DeleteCheckInRemarkInput.parse(input);
