...
export function EditMemberForm({ member, sectors }: EditMemberFormProps) {
  ...
  // Prepare representatives for display with metadata
  const reps = form.store.state.values.representatives || [];
  const representativesMeta = reps.map((rep, index) => ({
    keyRepresentative: rep,
    index,
    title:
      index === 0 ? "Principal Representative" : "Alternate Representative",
  }));

  const hasAlternate = representativesMeta.length > 1;

  const addAlternate = () => {
    const currentReps = form.getFieldValue("representatives") || [];
    form.setFieldValue("representatives", [
      ...currentReps,
  ...
  const removeAlternate = () => {
    ...
    if (currentReps.length > 1) {
      form.setFieldValue("representatives", [currentReps[0]]);
    }
  };
        ...
        <div className="space-y-6">
          <Card className="w-full overflow-hidden rounded-2xl bg-transparent shadow-none ring-0">
            ...
            <CardContent className="space-y-6 px-0 sm:px-6">
              ...
              <form.AppField name="companyAddress">
                {(field) => <field.TextareaField label="Company Address" />}
              </form.AppField>
          ...
          <Card className="w-full overflow-hidden rounded-2xl bg-transparent shadow-none ring-0">
            <CardHeader className="border-border/30 border-b bg-card/5 pb-4 sm:pb-6">
              ...
              <CardDescription className="text-muted-foreground text-sm">
                Update member details and representative information.
              </CardDescription>
            </CardHeader>

            <CardContent className="space-y-5 px-0 sm:space-y-6 sm:px-6">
              {representativesMeta.map((representativeMeta) => (
                <Card
          ...
          <Card className="w-full overflow-hidden rounded-2xl bg-transparent shadow-none ring-0">
            <CardHeader className="border-border/30 border-b bg-card/5 pb-4 sm:pb-6">
              ...
              <CardDescription className="text-muted-foreground text-sm">
                Review and update membership status and dates.
              </CardDescription>
            </CardHeader>
