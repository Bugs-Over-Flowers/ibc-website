...
export function setActiveEvent(
  ...
  world.activeEventAlias = alias;
  world.activeEventId = event.event.eventId;
  world.activeEventTitle = event.event.eventTitle;
}

export function getActiveEventOrThrow(world: RegistrationWorld): {
  eventId: string;
  eventTitle: string;
} {
  if (!world.activeEventId || !world.activeEventTitle) {
    throw new Error("No active event selected for this scenario.");
  }

  return {
    eventId: world.activeEventId,
    eventTitle: world.activeEventTitle,
  };
}

export function createUniqueRegistrantEmail(): string {
  return `e2e.registrant.${Date.now()}.${Math.floor(Math.random() * 10000)}@test.local`;
}
...
export async function fillValidStep2Registrant(
  ...
  const email = createUniqueRegistrantEmail();
...
export async function selectPaymentMethod(
  ...
  await page.locator("#onsite").click();
}
